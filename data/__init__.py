...
def get_spectral_response(data_name, srf_name):
    ...
    # sp_data = sp_data[0:103,0:4] #读取sp矩阵
    # sp_data = sp_data[:16,:]
    sp_data = sp_data / (sp_data.sum(axis=0))
    return sp_data

def create_dataset(arg, sp_matrix, isTRain,train_type):
    dataset_instance = Dataset(arg, sp_matrix, isTRain,train_type)
...
class DatasetDataLoader():
    def init(self, arg, isTrain=True):
        ...
        self.train_type = arg.train_type
        self.dataset = create_dataset(arg, self.sp_matrix, isTrain,self.train_type)
