...
class CUNets(BaseModel):
    ...
    def initialize(self, opt, hsi_channels, msi_channels, lrhsi_hei, lrhsi_wid, sp_matrix, sp_range):
        ...
        # net getnerator
        # hsi
        # self.net_LR_1 = network.my_define_lr_1(input_ch=hsi_channels, gpu_ids=self.gpu_ids, ngf=ngf)
        # self.net_LR_2 = network.my_define_lr_2(output_ch=num_s, gpu_ids=self.gpu_ids, useSoftmax=opt.useSoftmax,
        #                                        ngf=ngf)
        # self.net_reconstruct_lrhsi = network.define_s2img(input_ch=num_s, output_ch=hsi_channels, gpu_ids=self.gpu_ids)
        # self.net_reconstruct_lrhsi_transformer = network.define_s2img(input_ch=1024, output_ch=num_s, gpu_ids=self.gpu_ids)
        self.net_bl_lr = network.define_bm_net(temperature=32, gpu_ids= self.gpu_ids)  # 双线性 MTM
        self.net_reclr = network.define_s2img_2stream(input_c1 = num_s, input_tf = self.net_bl_lr, output_ch=hsi_channels,
                                                      gpu_ids= self.gpu_ids)
        # MSI
        # self.net_MSI_1 = network.my_define_msi_1(input_ch= msi_channels, gpu_ids=self.gpu_ids, ngf=ngf)
        # self.net_MSI_2 = network.my_define_msi_2(output_ch= num_s, gpu_ids=self.gpu_ids, ngf=ngf,
        ...
        self.net_bl_msi = network.define_bm_net(temperature=16, gpu_ids=self.gpu_ids)
        self.net_recmsi = network.define_s2img_2stream_msi(input_c1= num_s, input_tf=self.net_bl_msi, output_ch=msi_channels,
                                                       gpu_ids=self.gpu_ids)
        # self.net_rechsi = network.define_s2img_2stream(input_c1= num_s, input_tf=self.net_bl_lr, output_ch=hsi_channels,
        #                                               gpu_ids=self.gpu_ids)
        # self.net_bl_hsi = network.bm_net(num_s, temperature=1, gpu_ids=self.gpu_ids)
        #
        self.net_Displacefiled = network.define_displacementfiled(2 * num_s, 2, gpu_ids=self.gpu_ids)
        self.net_Spacetransform = network.define_spatial_transform(lr_size)
        # self.net_affinenet = network.define_TTSR(num_s, hsi_bands=num_s, msi_bands=num_s,
        #                                          gpu_ids=self.gpu_ids)  # hsi_bands 与net_LR_2输出通道数保持相同，msi_bands 与self.net_MSI_2保持一致
    ...
    def setup_optimizers(self, lr=None):
        ...
        # self.optimizer_MSI_1 = torch.optim.Adam(itertools.chain(self.net_MSI_1.parameters()), lr=lr * 0.5,
        #                                         betas=(0.9, 0.999))
        # self.optimizers.append(self.optimizer_MSI_1)
        # self.optimizer_MSI_2 = torch.optim.Adam(itertools.chain(self.net_MSI_2.parameters()), lr=lr * 0.5,
        #                                         betas=(0.9, 0.999))
        # self.optimizers.append(self.optimizer_MSI_2)
        # self.optimizer_ms2img = torch.optim.Adam(itertools.chain(self.net_reconstruct_hrhsi.parameters()), lr=lr,
        #                                          betas=(0.9, 0.999))
        # self.optimizers.append(self.optimizer_ms2img)
        ...
        # self.optimizers_G_rechsi = torch.optim.Adam(itertools.chain(self.net_rechsi.parameters()), lr=lr,
        #                                             betas=(0.9, 0.999))
        # self.optimizers.append(self.optimizers_G_rechsi)
        # self.optimizers_G_affinenet = torch.optim.Adam(itertools.chain(self.net_affinenet.parameters()), lr=lr * 0.1,
        #                                                betas=(0.9, 0.999))
        # self.optimizers.append(self.optimizers_G_affinenet)

        # self.optimizer_LR2s_1 = torch.optim.Adam(itertools.chain(self.net_LR_1.parameters()), lr=lr, betas=(0.9, 0.999))
        # self.optimizers.append(self.optimizer_LR2s_1)
        # self.optimizer_LR2s_2 = torch.optim.Adam(itertools.chain(self.net_LR_2.parameters()), lr=lr, betas=(0.9, 0.999))
        # self.optimizers.append(self.optimizer_LR2s_2)
        # self.optimizer_s2img = torch.optim.Adam(itertools.chain(self.net_reconstruct_lrhsi.parameters()), lr=lr,
        #                                         betas=(0.9, 0.999))
        # self.optimizers.append(self.optimizer_s2img)
        # 0.2
    ...
    def my_forward(self):
        ...
        # self.my_rec_lr_s_1 = self.net_LR_1(self.real_lhsi)
        # self.my_rec_msi_s_1 = self.net_MSI_1(self.real_hmsi)
        #
        # # self.lr_abundance = self.net_LR_2(torch.cat(
        # #     [self.my_rec_lr_s_1, torch.mul(self.my_rec_lr_s_1, self.net_PSF_2(self.net_mut_spa(self.my_rec_msi_s_1)))],
        # #     dim=1))
        # # self.msi_abundance = self.net_MSI_2(
        # #     torch.cat([self.my_rec_msi_s_1, torch.mul(self.my_rec_msi_s_1, self.net_mut_spe(self.my_rec_lr_s_1))],
        # #               dim=1))
        # self.lr_abundance = self.net_LR_2(
        #     self.my_rec_lr_s_1)
        # self.msi_abundance = self.net_MSI_2(
        #     self.my_rec_msi_s_1)
        ...
        # self.aff_lrhsi = self.net_PANet(self.lr_abundance, self.d1, self.d2, self.d3)
        # self.aff_lrhsi, _, _, self.H = self.net_affinenet(self.lr_abundance, self.msi_abundance,
        #                                                   self.msi_abundance)  # TTSR
        # # #! the shape of recovered abundance is [batchsize, y, x, num_p]
        # self.rec_RecoveredAbundance = self.net_Spacetransform(
        #     self.lr_abundance.permute(0, 2, 3, 1), self.rec_DisplacementField
        # )
        # # self.aff_lrhsi =self.net_Merge(self.rec_RecoveredAbundance.permute(0,3,1,2),
        # #     torch.mul(self.net_PSF_3(self.msi_abundance), self.net_mut_spa(self.rec_RecoveredAbundance.permute(0, 3, 1, 2))))
        # self.aff_lrhsi = self.rec_RecoveredAbundance.permute(0,3,1,2)

        # self.aff_lrhsi, _, _, self.H = self.net_affinenet(self.rec_RecoveredAbundance.permute(0,3,1,2).contiguous(), self.msi_abundance,self.msi_abundance)  # TTSR
        # self.aff_lrhsi = self.rec_RecoveredAbundance.permute(0,3,1,2).contiguous()

        # self.aff_lrhsi = self.rec_RecoveredAbundance.permute(0, 3, 1, 2)+self.net_PSF_3(
        #     torch.mul(self.msi_abundance, self.net_mut_spe(self.rec_RecoveredAbundance.permute(0, 3, 1, 2))))
        # self.rec_lrhsi  = self.net_reclr(self.lr_abundance)
        self.rec_lrhsi,self.lr_abundance_bi = self.net_reclr(self.lr_abundance, None)  # 重构的lrhsi
        # self.rec_aff_lrhsi = self.net_reclr(self.rec_RecoveredAbundance.permute(0,3,1,2))  # 用校正后的
        self.rec_aff_lrhsi,_= self.net_reclr(self.aff_lrhsi, None)
        # save_path = "my_rec_lr.mat"
        # A = self.rec_lrhsi
        # io.savemat(save_path, {"Reclr": A.detach().cpu().numpy()})
        ...
        self.rec_lrhsi_lrmsi = self.net_HR2MSI(self.real_lhsi)  # 真实的LRHSI 光谱下采样为LRMSI
        self.rec_hrmsi_lrmsi = self.net_PSF(self.real_hmsi)  # 真实的HRMSI 空间下采样为LRMSI
        # self.rec_lrhsi_lrmsi = self.net_HR2MSI(self.rec_aff_lrhsi)  # 真实的LRHSI 光谱下采样为LRMSI
        # self.rec_hrmsi_lrmsi = self.net_PSF(self.rec_hrmsi)  # 真实的HRMSI 空间下采样为LRMSI
