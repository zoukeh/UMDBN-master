...
def define_s2img_2stream(input_ch, output_ch,gpu_ids, init_type='kaiming', init_gain=0.02):
    net = S2Img_2stream(input_c=input_ch, output_c=output_ch)
    return init_net(net, init_type, init_gain, gpu_ids)
def define_transformer(input_ch, temperature,gpu_ids, init_type='kaiming', init_gain=0.02):
    net = Transformer(input_ch,input_ch,temperature)
    return init_net(net, init_type, init_gain, gpu_ids)
...
#        return (self.net1(x1)+self.net2(torch.pow(x2,2))).clamp_(0, 1)   
        return (self.net1(x1)+self.net2(x2)).clamp_(0, 1)   
