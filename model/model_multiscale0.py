...
class Registration(nn.Module):
    ...
    def expansive_block(self, in_channels, mid_channel, out_channels, kernel_size=3):
        ...
        block = torch.nn.Sequential(
            ...
            torch.nn.LeakyReLU(0.2), #75的时候去掉
        )
        return block
...
class PANet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv5 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1) #3 1
    ...
    # def forward(self, ori_lr_hsi, d1, d2, d3):
        ...
        # x2 = F.relu(self.conv2(torch.cat((d2,self.upsample1(d1)),dim=1)))
        # x3 = F.relu(self.conv3(torch.cat((d3,self.upsample1(d2)),dim=1)))
        # x4 = F.relu(self.conv4(torch.cat((d4,self.upsample1(d3)),dim=1)))
